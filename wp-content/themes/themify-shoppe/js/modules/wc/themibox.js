...
(($,Themify, doc) =>{
    ...
    const pattern = doc.createElement('div'),
        ...
        loadCss=()=>{
            return new Promise((resolve, reject) => {
                const js=themifyScript.wc_gal,
                    concateCss=doc.getElementById('themify_concate-css')?.nextElementSibling,
					checkLoad =   ()=> {
        ...
        clickLightBox = function(e) {
            ...
            product = clickedItem.closest('.product,.slide-inner-wrap,.type-product')?.querySelector('.product-image,.post-image');
            if (!product) {
                return false;
            }
                            ...
                            ajax.then(resp=> {
													...
													wrap.addEventListener('transitionend', function(e) {
														...
														wrapCl.remove('tf_opacity');
															...
															if (typeof ThemifyShoppeAjaxCart === 'function') {
																container.querySelector('form.cart')?.addEventListener('submit', ThemifyShoppeAjaxCart);
															}
                                ...
                                let wrapClone=wrap.cloneNode(true),
                                    ...
                                    if (Themify.cssLazy['theme_social_share'] !== true && cloneContainer.getElementsByClassName('share-wrap')[0]) {
										...
										Themify.LoadCss(cssUrl + 'social-share.css', themify_vars.theme_v, doc.getElementById('themify_concate-css')?.nextElementSibling);
                                    }
