...
<div <?php echo self::get_element_attributes(self::sticky_element_props($container_props,$fields_args)); ?>>
	...
	<?php foreach($orders as $k=>$i):?>
		...
		?>
		    <div class="builder-contact-field builder-contact-field-<?php echo $k,($k==='message'?' builder-contact-textarea-field':' builder-contact-text-field')?>">
			    <label class="control-label" for="<?php echo $args['module_ID']; ?>-contact-<?php echo $k?>"><span class="tb-label-span"<?php if($is_inline_edit_supported===true){ self::add_inline_edit_fields('field_'.$k.'_label'); }?>><?php if ($label!== ''): ?><?php echo $label; ?> </span><?php if ( $required ) : ?><span class="required">*</span><?php endif; endif; ?></label>
			    <div class="control-input tf_rel">
				    ...
				    <?php else:?>
					    <input type="text" name="contact-<?php echo $k?>" placeholder="<?php echo $placeholder; ?>" id="<?php echo $args['module_ID']; ?>-contact-<?php echo $k?>" value="" class="form-control" <?php echo $required===true ? 'required' : '' ?>>
				    <?php endif;?>
	...
	<?php endforeach;?>
	    ...
	    <div class="builder-contact-field builder-contact-field-send control-input tf_text<?php echo $fields_args['field_send_align'][0];?> tf_clear tf_rel">
			<button type="submit" class="btn btn-primary"<?php if($is_inline_edit_supported===true){ self::add_inline_edit_fields('field_send_label'); }?>><?php if(Themify_Builder::$frontedit_active===false):?><span class="tf_loader"></span><?php endif;?><?php echo $fields_args['field_send_label']; ?></button>
	    </div>
	</div>
    </form>
