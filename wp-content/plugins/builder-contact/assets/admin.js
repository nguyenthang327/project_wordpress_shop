(function ($) {
    ...
    contactFormBuilder.prototype = {
            ...
            this.$table
                    ...
                    .on('click.tb_contact', '.tb_contact_value_remove,.tb_contact_field_remove', function (e) {
						...
						if (this.classList.contains('tb_contact_value_remove')) {
							...
							let row = $(this).closest('.tb_contact_new_row'),
								name = row.find( '.tb_new_field_textbox' ).val().trim();
                            row.remove();
							// remove the corresponding template tag
							[...ThemifyBuilderCommon.Lightbox.$lightbox[0].querySelectorAll( '.tb_contact_custom_tags span' )].filter( function( el ) {
        ...
        addField(data) {
			...
			let templateTag = document.createElement( 'span' );
			templateTag.appendChild( document.createTextNode( '%' + name.value + '%' ) );
			ThemifyBuilderCommon.Lightbox.$lightbox[0].querySelector( '.tb_contact_custom_tags' ).appendChild( templateTag );
        ...
        render(data, self) {
            ...
            document.addEventListener('tb_editing_contact', function (e) {
				const $lightbox = ThemifyBuilderCommon.Lightbox.$lightbox,
					$template = $lightbox.find( '#template' );
				if ( $template.val() === '' ) {
					$template.val( tb_contact_l10n['default_template'] );
				}
				...
				$lightbox[0].querySelector( '.template_fields' ).addEventListener( 'click', function( e ) {
					if ( e.target.tagName.toLocaleLowerCase() === 'span' ) {
						$lightbox[0].querySelector( '#template' ).value += e.target.textContent;
					}
				} );
            }, {once: true});
	...
	tb_app.Forms.register_validator( 'tb_contact_recipient', function( value ) {
		let send_to = window.top.document.getElementById( 'send_to_admins' ).querySelector( '.tb-checkbox' );
		if ( send_to.checked ) {
			return true;
		}
		if ( value.trim() === '' ) {
			...
			value = value.split( ',' ).map( ( str ) => { return str.trim() } );
			return value.every( ( item ) => {
				/* validate email address */
				return tb_app.Forms.get_validator('email')(item);
			} );
		}
